...
public class InrixIncidentListFragment extends ListFragment {
	...
	private void validateList(Place currentLocation, List<Incident> incidents) {
		...
		while (iterator.hasNext()) {
			...
			if (current.getFullDescription() == null) {
				...
				current.setDistanceKM(10.9);
			}
		}

		Collections.sort(incidents, IncidentUtils.getDefaultListComparator());
	...
	private class InrixModelListAdapter extends BaseAdapter {
		 ...
		 * 
		 * @param holder
		 *            Holder to display the description.
		 * @param incident
		 *            Incident to take the description.
		 */
		private void setDescription(ViewHolder holder, Incident incident) {
			...
			// Uses geocoder to retrieve address of incident.
			AddressLocator locator = new AddressLocator(context,
					new AddressLocatorCallback(incident.getId()));
			locator.getAddress((float) incident.getLatitude(),
		...
		private class AddressLocatorCallback implements
				AddressLocatorListCallback {

			private long incidentId;

			public AddressLocatorCallback(long incidentId) {
			...
			public void onAddressListFound(List<Address> addressList) {
				...
				if (addressList != null && addressList.size() > 0
						&& (address = addressList.get(0)) != null) {
					...
					for (int i = count; i >= 0; i--) {
						...
						addressStringBuilder.append(addressLine);
					}

					descriptionMap.put(this.incidentId,
			...
			/**
			 * Sets description text when geocoder was failed or nothing was
			 * found.
			 */
