 ...
 * within it).
 */
public class DefaultIncidentsClusteringAlgorithm<T extends ClusterItem>
	 ...
	 * Any modifications should be synchronized on mQuadTree.
	 */
	private final PointQuadTree<QuadItem<T>> mQuadTree = new PointQuadTree<QuadItem<T>>(0,
			1,
			0,
			1);
	...
	public Set<? extends Cluster<T>> getClusters(double zoom) {
		...
		synchronized (mQuadTree) {
			for (QuadItem<T> candidate : mItems) {
				...
				if (!enableClustering) {
					// don't need to cluster - just shortcut
					results.add(candidate);
					visitedCandidates.add(candidate);
					continue;
				}

				Bounds searchBounds = createBoundsFromSpan(candidate.getPoint(),
						zoomSpecificSpan);
				...
				StaticCluster<T> cluster = new StaticCluster<T>(candidate.mClusterItem
						.getPosition());
				results.add(cluster);
				...
				for (QuadItem<T> clusterItem : clusterItems) {
					...
					if (existingDistance != null) {
						...
						itemToCluster.get(clusterItem)
								.remove(clusterItem.mClusterItem);
					}
